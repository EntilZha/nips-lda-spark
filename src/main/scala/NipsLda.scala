...
object NipsLda {
  ...
  def edgesVocabFromEdgeListDictionary(sc:SparkContext):
                                      (RDD[(LDA.WordId, LDA.DocId)], Array[String], Map[String, LDA.WordId]) = {
    val doc = sc.textFile("/root/spark/nips-lda-spark/data/numeric-nips/counts.tsv")
    ...
    val vocab = io.Source.fromFile("/root/spark/nips-lda-spark/data/numeric-nips/dictionary.txt").getLines().toArray
